...
#define CG_6M_MUL_6_664_FPLL    (0x001A7535UL<<8U)      /* fPLL = 6MHz  * 6.664    */
#define CG_8M_MUL_5_FPLL        (0x00247028UL<<8U)      /* fPLL = 8MHz  * 5        */
#define CG_10M_MUL_4_FPLL       (0x002C7020UL<<8U)      /* fPLL = 10MHz * 4        */
...
#define WUPHCR_WUPT_EXT        ((uint32_t)(((uint64_t)WU_TIME_EXT * EXTALH / HZ_1M /16UL) << 20U)) /* OSCCR<WUPT11:0> = warm-up time(us) * EXTALH / 16 */
#if (CLOCK_SETUP)
  #define WUPHCR_WUPT_PLL        ((WU_TIME_PLL * EXTALH / HZ_1M /16UL) << 20U)
#else
  #define WUPHCR_WUPT_PLL        ((WU_TIME_PLL * IXTALH / HZ_1M /16UL) << 20U)
#endif
...
void SystemInit(void)          
{
    ...
    TSB_SIWD0->EN  = 0x00000000;  /* Clear Enable bit.     */
    TSB_SIWD0->CR  = 0x000000B1;  /* Write Disable Code.   */  
    TSB_SIWD0->MOD = 0x00000002;  /* Switch to INTWD(NMI). */ 
    TSB_SIWD0->EN  = 0x00000001;  /* Set Enable bit.       */
    ...
    TSB_CG->WUPHCR = (WUPHCR_WUPT_PLL | CG_WUPHCR_WUCLK_SET);
    TSB_CG->PLL0SEL &= CG_PLL0SEL_PLL0SEL_CLEAR;    /* PLL-->fOsc */
    TSB_CG->PLL0SEL &= CG_PLL0SEL_PLL0ON_CLEAR;
    TSB_CG->PLL0SEL = PLL0SEL_Ready;
    TSB_CG->WUPHCR = (WUPHCR_WUPT_PLL | CG_WUPHCR_WUCLK_SET | CG_WUPHCR_WUON_START_SET);
    while (TSB_CG_WUPHCR_WUEF) {
        ;
    }                           /* Warm-up */
